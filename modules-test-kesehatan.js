  ...
  if (!filtered.length) {
    ...
    filtered.forEach((p) => {
      const kesimpulanStatus =
        p.kesimpulan && p.kesimpulan.status ? getStatusBadgeKesehatan(p.kesimpulan.status) : '-';
      const statusBadge =
        p.status === 'selesai'
...
async function simpanJadwalTestKesehatan(tipe) {
  ...
  if (tipe === 'calon') {
    var shareUrl = window.location.origin + '/test-kesehatan?id=' + newDocRef.id;
    openModal(`
  ...
  try {
    ...
    if (id && id.trim()) {
      ...
      try {
        ...
        console.warn('[TEST KESEHATAN] Update failed, trying set with merge:', updateErr.message);
        await db.collection('hrd_test_kesehatan').doc(id).set(updateData, { merge: true });
      }
    } else {
