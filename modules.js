...
async function renderDashboard() {
  ...
  main.innerHTML='<div class="page-title"><span>[CHART] Dashboard</span></div><div class="stats-grid" id="dashStats">Loading...</div><div class="grid-2" id="dashWidgets"></div>';
  ...
  document.getElementById('dashStats').innerHTML=`
    <div class="stat-card"><div class="stat-icon">[USERS]</div><div class="stat-value">${karyawan.size}</div><div class="stat-label">Total Karyawan</div></div>
    <div class="stat-card"><div class="stat-icon">[PIN]</div><div class="stat-value">${absen.size}</div><div class="stat-label">Hadir Hari Ini</div></div>
    <div class="stat-card"><div class="stat-icon">[LEAVE]</div><div class="stat-value">${cuti.size}</div><div class="stat-label">Cuti Pending</div></div>
    <div class="stat-card"><div class="stat-icon">[ANNOUNCE]</div><div class="stat-value">${pengumuman.size}</div><div class="stat-label">Pengumuman</div></div>`;
  let aHtml='<div class="card"><div class="card-title">[ANNOUNCE] Pengumuman Terbaru</div>';
...
// ── POSISI ────────────────────────────────────────────────────
async function renderPosisi(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[JOB] Posisi</span><div class="flex gap-8"><button class="btn btn-primary btn-sm" onclick="modalPosisi()">+ Tambah</button><button class="btn btn-info btn-sm" onclick="syncPosFromKaryawan()">[SYNC] Sinkron dari Karyawan</button></div></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Nama</th><th>Departemen</th><th>Level</th><th>Aksi</th></tr></thead><tbody id="tblPos"></tbody></table></div></div>`;const snap=await db.collection('hrd_posisi').orderBy('nama').get();let h='';if(snap.empty)h='<tr><td colspan="4" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.departemen||'-')}</td><td>${escHtml(p.level||'-')}</td><td><button class="btn btn-xs btn-info" onclick="modalPosisi('${d.id}')">[EDIT]</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_posisi','${d.id}','posisi')">[DEL]</button></td></tr>`;});document.getElementById('tblPos').innerHTML=h;}
...
async function lihatHistoryKontrak(karyawanId){
  ...
  let h=`<div class="modal-title">[LIST] History Kontrak Kerja</div>`;
...
function modalImportKaryawan(){openModal(`<div class="modal-title">[IMPORT] Import Data Karyawan</div>
    <div class="tabs mb-16"><div class="tab active" onclick="switchImportTab('kary','file')">[DOC] Upload CSV</div><div class="tab" onclick="switchImportTab('kary','api')">[LINK] API Google Sheets</div></div>
    <div id="importKaryTab">
      ...
      <div class="flex gap-8 mb-16"><button class="btn btn-primary" onclick="processImportKaryawan()">[IMPORT] Proses Import</button><button class="btn btn-outline btn-sm" onclick="downloadKaryawanTemplate()">[DOC] Download Template</button></div>
...
function switchImportTab(type, mode){
  ...
  if(mode==='api'){
    el.innerHTML=`<div style="padding:12px;background:#e8f5e9;border-radius:8px;margin-bottom:16px;border-left:4px solid var(--success)">
        <div class="fw-700 text-sm mb-8">[OK] Cukup paste link Google Sheets biasa!</div>
...
async function previewApiImport(type){
  ...
  document.getElementById('importApiStatus').innerHTML='<span class="badge badge-warning">[WAIT] Mengambil data dari Google Sheets...</span>';
  try{
    ...
    if(rows.length<2){document.getElementById('importApiStatus').innerHTML='<span class="badge badge-danger">[X] Data kosong atau format tidak valid</span>';return;}
    document.getElementById('importApiStatus').innerHTML=`<span class="badge badge-success">[OK] ${rows.length-1} baris data ditemukan (${rows[0].length} kolom)</span>`;
    ...
    document.getElementById('importApiStatus').innerHTML=`<span class="badge badge-danger">[X] Gagal: ${e.message}</span>
...
async function processImportKaryawanFromText(text){
  ...
  closeModalDirect();toast(`[OK] Import selesai: ${added} baru, ${updated} terupdate`,'success');renderKaryawan();
}
...
async function renderStrukturOrg(){
  const main=document.getElementById('mainContent');
  main.innerHTML=`<div class="page-title"><span>[TREE] Struktur Organisasi</span></div><div class="card" id="orgChart" style="overflow-x:auto;padding:30px 20px">Loading...</div>`;
...
async function simpanCuti(){
  ...
  if(data.atasan){
    ...
    if(!atasanSnap.empty)await sendNotification(atasanSnap.docs[0].id,'[LIST] Pengajuan Cuti',`${data.nama} mengajukan ${data.jenis} (${durasi} hari)`,'approval-center');
  }
  await sendNotification('hr','[LIST] Pengajuan Cuti',`${data.nama} mengajukan ${data.jenis}`,'approval-center');
...
// ── PENGGAJIAN ────────────────────────────────────────────────
async function renderPenggajian(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[MONEY] Penggajian</span><div><button class="btn btn-primary btn-sm" onclick="modalGaji()">+ Generate Slip</button> <button class="btn btn-secondary btn-sm" onclick="modalImportPenggajian()">[DL] Import</button></div></div><div class="card"><div class="flex gap-8 mb-16"><input class="form-control" type="month" id="filterBulanGaji" value="${monthStr()}" onchange="loadGaji()"><button class="btn btn-sm btn-info" onclick="loadGaji()">[FIND]</button></div><div class="table-wrap"><table><thead><tr><th>Karyawan</th><th>Gaji Pokok</th><th>Tunjangan</th><th>Potongan</th><th>PPH21</th><th>Total</th><th>Aksi</th></tr></thead><tbody id="tblGaji"></tbody></table></div></div>`;loadGaji();}
async function loadGaji(){const bulan=document.getElementById('filterBulanGaji')?.value||monthStr();const snap=await db.collection('hrd_penggajian').where('periode','==',bulan).get();let h='';if(snap.empty)h='<tr><td colspan="7" class="text-center">Belum ada slip</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${formatCurrency(p.gajiPokok)}</td><td>${formatCurrency(p.tunjangan)}</td><td>${formatCurrency(p.potongan)}</td><td>${formatCurrency(p.pph21)}</td><td class="fw-700">${formatCurrency(p.totalBersih)}</td><td><button class="btn btn-xs btn-info" onclick="lihatSlip('${d.id}')">[DOC]</button> <button class="btn btn-xs btn-warning" onclick="editGaji('${d.id}')">[EDIT]</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_penggajian','${d.id}','penggajian')">[DEL]</button></td></tr>`;});document.getElementById('tblGaji').innerHTML=h;}
...
function modalImportPenggajian(){openModal(`<div class="modal-title">[IMPORT] Import Data Penggajian</div>
    <div class="tabs mb-16"><div class="tab active" onclick="switchImportTab('gaji','file')">[DOC] Upload CSV</div><div class="tab" onclick="switchImportTab('gaji','api')">[LINK] API Google Sheets</div></div>
    <div id="importGajiTab">
      ...
      <div class="flex gap-8 mb-16"><button class="btn btn-primary" onclick="processImportPenggajian()">[IMPORT] Proses Import</button><button class="btn btn-outline btn-sm" onclick="downloadPenggajianTemplate()">[DOC] Download Template</button></div>
      <div class="text-xs" style="color:#666">Template CSV akan membantu memastikan header sesuai dengan format sheet.</div>
    </div>`,true);}
...
async function processImportPenggajianFromText(text){
  ...
  closeModalDirect();toast(`[OK] Import selesai: ${added} baru, ${updated} terupdate`,'success');renderPenggajian();
}
...
// ── TUNJANGAN ─────────────────────────────────────────────────
async function renderTunjangan(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[GIFT] Tunjangan & Benefit</span><button class="btn btn-primary btn-sm" onclick="modalTunjangan()">+ Tambah</button></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Nama</th><th>Jenis</th><th>Nominal</th><th>Penerima</th><th>Aksi</th></tr></thead><tbody id="tblTunj"></tbody></table></div></div>`;const snap=await db.collection('hrd_tunjangan').orderBy('nama').get();let h='';if(snap.empty)h='<tr><td colspan="5" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.jenis||'-')}</td><td>${formatCurrency(p.nominal)}</td><td>${escHtml(p.penerima||'Semua')}</td><td><button class="btn btn-xs btn-info" onclick="modalTunjangan('${d.id}')">[EDIT]</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_tunjangan','${d.id}','tunjangan')">[DEL]</button></td></tr>`;});document.getElementById('tblTunj').innerHTML=h;}
...
// ── PELATIHAN ─────────────────────────────────────────────────
async function renderPelatihan(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[GRAD] Pelatihan & Sertifikasi</span><button class="btn btn-primary btn-sm" onclick="modalPelatihan()">+ Tambah</button></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Judul</th><th>Jenis</th><th>Tanggal</th><th>Peserta</th><th>Status</th><th>Aksi</th></tr></thead><tbody id="tblPelatihan"></tbody></table></div></div>`;const snap=await db.collection('hrd_pelatihan').orderBy('createdAt','desc').get();let h='';if(snap.empty)h='<tr><td colspan="6" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td class="fw-700">${escHtml(p.judul)}</td><td>${escHtml(p.jenis)}</td><td>${formatDate(p.tanggal)}</td><td>${(p.peserta||[]).length}</td><td><span class="badge badge-${p.status==='selesai'?'success':'info'}">${p.status||'terjadwal'}</span></td><td><button class="btn btn-xs btn-info" onclick="modalPelatihan('${d.id}')">[EDIT]</button></td></tr>`;});document.getElementById('tblPelatihan').innerHTML=h;}
...
// ── KONTRAK ───────────────────────────────────────────────────
async function renderKontrak(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[DOC] Kontrak & Perjanjian</span><button class="btn btn-primary btn-sm" onclick="modalKontrak()">+ Tambah</button></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Judul</th><th>Pihak</th><th>Mulai</th><th>Berakhir</th><th>Status</th><th>Aksi</th></tr></thead><tbody id="tblKontrak"></tbody></table></div></div>`;const snap=await db.collection('hrd_kontrak').orderBy('berakhir').get();const today=todayStr();let h='';if(snap.empty)h='<tr><td colspan="6" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();const expired=p.berakhir<today;h+=`<tr><td class="fw-700">${escHtml(p.judul)}</td><td>${escHtml(p.pihak||'-')}</td><td>${formatDate(p.mulai)}</td><td>${formatDate(p.berakhir)}</td><td><span class="badge badge-${expired?'danger':'success'}">${expired?'Expired':'Aktif'}</span></td><td><button class="btn btn-xs btn-info" onclick="modalKontrak('${d.id}')">[EDIT]</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_kontrak','${d.id}','kontrak')">[DEL]</button></td></tr>`;});document.getElementById('tblKontrak').innerHTML=h;}
...
// ── ASSET ─────────────────────────────────────────────────────
async function renderAsset(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[PC] Asset Management</span><button class="btn btn-primary btn-sm" onclick="modalAsset()">+ Tambah</button></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Kode</th><th>Nama</th><th>Kategori</th><th>Pengguna</th><th>Kondisi</th><th>Aksi</th></tr></thead><tbody id="tblAsset"></tbody></table></div></div>`;const snap=await db.collection('hrd_asset').orderBy('nama').get();let h='';if(snap.empty)h='<tr><td colspan="6" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td>${escHtml(p.kode||'-')}</td><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.kategori||'-')}</td><td>${escHtml(p.pengguna||'-')}</td><td><span class="badge badge-${p.kondisi==='baik'?'success':p.kondisi==='rusak'?'danger':'warning'}">${p.kondisi||'baik'}</span></td><td><button class="btn btn-xs btn-info" onclick="modalAsset('${d.id}')">[EDIT]</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_asset','${d.id}','asset')">[DEL]</button></td></tr>`;});document.getElementById('tblAsset').innerHTML=h;}
...
async function renderMeeting() {
  const main = document.getElementById('mainContent');
  main.innerHTML = `
    <div class="page-title"><span>[CAL] Meeting & Invite</span><button class="btn btn-primary btn-sm" onclick="modalMeetingCreate()">+ Buat Meeting</button></div>
    <div class="tabs">
      <div class="tab active" onclick="loadMeetingTab('saya')">📤 Meeting Saya (Dibuat)</div>
      <div class="tab" onclick="loadMeetingTab('semua')">[LIST] Semua Meeting</div>
    </div>
...
async function loadMeetingTab(tab) {
  ...
  if (snap.empty) {
    html += '<div class="empty-state"><div class="icon">[CAL]</div><p>Belum ada meeting</p></div>';
  } else {
    ...
    snap.forEach(d => {
      ...
      html += `<tr>
        ...
        <td><button class="btn btn-xs btn-info" onclick="detailMeeting('${d.id}')">[VIEW]</button> <button class="btn btn-xs btn-warning" onclick="modalNotulensi('${d.id}')">[NOTE]</button></td>
      </tr>`;
    });
...
async function modalMeetingCreate() {
  ...
  openModal(`<div class="modal-title">[CAL] Buat Meeting & Undang Peserta</div>
    ...
    <div class="form-group"><label>[SEND] Undang Peserta (pilih per akun):</label>
    ...
    <button class="btn btn-primary" onclick="simpanMeeting()">[SEND] Simpan & Kirim Undangan</button>`, true);
}
...
function detailMeeting(id) {
  db.collection('hrd_meeting').doc(id).get().then(d => {
    ...
    openModal(`<div class="modal-title">[CAL] ${escHtml(p.judul)}</div>
...
function modalNotulensi(id) {
  db.collection('hrd_meeting').doc(id).get().then(d => {
    const p = d.data();
    openModal(`<div class="modal-title">[NOTE] Notulensi: ${escHtml(p.judul)}</div>
...
async function renderInbox() {
  const main = document.getElementById('mainContent');
  main.innerHTML = `<div class="page-title"><span>[INBOX] Inbox Saya</span></div><div class="card" id="inboxList">Loading...</div>`;
  ...
  const unsub = db.collection('hrd_meeting_invites')
    ...
    .onSnapshot(snap => {
      let html = '';
      if (snap.empty) {
        html = '<div class="empty-state"><div class="icon">[INBOX]</div><p>Inbox kosong — belum ada undangan meeting</p></div>';
      } else {
        snap.forEach(d => {
          const p = d.data();
          ...
          html += `<div class="inbox-item ${isUnread?'unread':''}" onclick="openInviteDetail('${d.id}')">
            <div style="display:flex;justify-content:space-between;align-items:center">
              <div>
                <div class="inbox-from">[CAL] ${escHtml(p.judul)}</div>
...
async function openInviteDetail(inviteId) {
  ...
  openModal(`<div class="modal-title">[CAL] Undangan Meeting</div>
    ...
    <div class="flex gap-8">
      <button class="btn btn-success" onclick="rsvpInvite('${inviteId}','${inv.meetingId}','hadir')">[OK] Hadir</button>
      <button class="btn btn-danger" onclick="rsvpInvite('${inviteId}','${inv.meetingId}','tidak')">[X] Tidak Hadir</button>
    </div>`, true);
}
...
async function rsvpInvite(inviteId, meetingId, status) {
  ...
  const invDoc = await db.collection('hrd_meeting_invites').doc(inviteId).get();
  const inv = invDoc.data();
  await sendNotification(inv.fromUser, '[CAL] RSVP Meeting', `${currentUser.nama} ${status==='hadir'?'akan hadir':'tidak hadir'} di meeting "${inv.judul}"`, 'meeting');
...
async function renderChat() {
  const main = document.getElementById('mainContent');
  main.innerHTML = `
    <div class="page-title"><span>[CHAT] Obrolan</span><button class="btn btn-primary btn-sm" onclick="modalNewChat()">+ Mulai Obrolan</button></div>
    <div class="grid-2">
      <div class="card">
        <div class="card-title mb-8">[SEND] Percakapan</div>
        <div id="chatThreadList" style="max-height:500px;overflow-y:auto">Loading...</div>
      </div>
      <div class="card" id="chatArea">
        <div class="empty-state"><div class="icon">[CHAT]</div><p>Pilih percakapan atau mulai obrolan baru</p></div>
      </div>
    </div>`;
  loadChatThreads();
}
  ...
  if (!sorted.length) {
    ...
    sorted.forEach(t => {
      ...
      html += `<div class="inbox-item ${unread?'unread':''}" onclick="openChatThread('${t.id}')">
        <div class="inbox-from">[CHAT] ${escHtml(otherName)}</div>
...
async function modalNewChat() {
  ...
  openModal(`<div class="modal-title">[CHAT] Mulai Obrolan Baru</div>
    ...
    <button class="btn btn-primary" onclick="startNewChat()">[SEND] Kirim</button>`);
}
...
async function startNewChat() {
  ...
  // Notify target user
  await sendNotification(toUser, '[CHAT] Pesan Baru', `${currentUser.nama}: ${msg.substring(0,80)}`, 'chat');
...
async function kirimBroadcast(){
  ...
  await sendNotificationBulk(targetIds,'[BROADCAST] Broadcast',`${currentUser.nama}: ${pesan.substring(0,100)}`,'notifikasi');
...
async function renderNotifikasi(){
  const main=document.getElementById('mainContent');
  main.innerHTML=`<div class="page-title"><span>[BELL] Notifikasi</span><button class="btn btn-sm btn-outline" onclick="markAllRead()">Tandai Semua Dibaca</button></div><div class="card" id="notifList">Loading...</div>`;
  ...
  let h='';
  if(!allNotifs.length)h='<div class="empty-state"><div class="icon">[BELL]</div><p>Tidak ada notifikasi</p></div>';
...
// ── PENGUMUMAN ────────────────────────────────────────────────
async function renderPengumuman(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>[ANNOUNCE] Pengumuman</span>${hasAccess(3)?'<button class="btn btn-primary btn-sm" onclick="modalPengumuman()">+ Tambah</button>':''}</div><div id="pengumumanList">Loading...</div>`;const snap=await db.collection('hrd_pengumuman').orderBy('createdAt','desc').get();let h='';if(snap.empty)h='<div class="empty-state"><div class="icon">[ANNOUNCE]</div><p>Belum ada</p></div>';else snap.forEach(d=>{const p=d.data();h+=`<div class="card"><div class="card-header"><div class="card-title">${escHtml(p.judul)}</div><div class="text-xs" style="color:#999">${formatDateTime(p.createdAt)}</div></div><div class="text-sm" style="white-space:pre-wrap">${escHtml(p.isi)}</div></div>`;});document.getElementById('pengumumanList').innerHTML=h;}
...
async function showAkunForm(id,p){
  ...
  openModal(`<div class="modal-title">${id?'Edit':'Tambah'} Akun</div>
    ...
    <div class="form-group" style="background:#f0f4ff;padding:12px;border-radius:8px;border-left:4px solid var(--primary)"><label style="color:var(--primary)">[LINK] Sambungkan ke Data Karyawan</label><select class="form-control" id="akLinkedKary">${karyOpts}</select><div class="text-xs mt-8" style="color:#666">Pilih karyawan yang terdaftar untuk menyinkronkan data (nama, departemen, posisi) secara otomatis.</div></div>
    <div class="flex gap-8 mt-16"><button class="btn btn-primary" onclick="simpanAkun('${id||''}')">[SAVE] Simpan</button>${id?`<button class="btn btn-danger" onclick="hapusDoc('hrd_users','${id}','akun')">[DEL] Hapus</button>`:''}</div>`,true);
}
...
async function renderApprovalCenter(){
  const main=document.getElementById('mainContent');
  main.innerHTML=`<div class="page-title"><span>[OK] Approval Center</span></div><div class="card" id="approvalList">Loading...</div>`;
  ...
  let h='';
  if(!items.length) h='<div class="empty-state"><div class="icon">[OK]</div><p>Tidak ada pengajuan pending</p></div>';
    ...
    h+=`<div style="padding:14px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      ...
      <div class="flex gap-8">
        <button class="btn btn-xs btn-primary" onclick="viewApprovalDetail('${item.collection}','${item.id}')">[VIEW] View</button>
        ${hasAccess(2)?`<button class="btn btn-xs btn-success" onclick="approveItem('${item.collection}','${item.id}','approved')">[OK]</button><button class="btn btn-xs btn-danger" onclick="approveItem('${item.collection}','${item.id}','rejected')">[X]</button>`:''}
      </div>
    </div>`;
  });
...
function viewApprovalDetail(col,id){
  db.collection(col).doc(id).get().then(d=>{
    ...
    let html=`<div class="modal-title">[LIST] Detail Pengajuan — ${type.toUpperCase()}</div>`;
    ...
    html+=`<div class="flex gap-8 mt-16">${hasAccess(2)?`<button class="btn btn-success" onclick="approveItem('${col}','${id}','approved')">[OK] Approve</button><button class="btn btn-danger" onclick="approveItem('${col}','${id}','rejected')">[X] Reject</button>`:''}</div>`;
    openModal(html,true);
  });
}
...
async function approveItem(col,id,status){
  ...
  const data=doc.data();
  if(data.userId)await sendNotification(data.userId,`${status==='approved'?'[OK] Disetujui':'[X] Ditolak'}`,`Pengajuan ${data.jenis||data.kategori||col.replace('hrd_','')} Anda telah ${status} oleh ${currentUser.nama}`);
  closeModalDirect();
...
function renderQRShare(){
  ...
  document.getElementById('mainContent').innerHTML=`<div class="page-title"><span>[PHONE] QR, PWA & Download Aplikasi</span></div>
    <div class="grid-2">
      <div class="card text-center">
        <div class="card-title mb-16">[PHONE] QR Code Aplikasi</div>
        ...
        <div class="flex gap-8 mt-8" style="justify-content:center">
          <button class="btn btn-primary btn-sm" onclick="navigator.clipboard?.writeText('${url}').then(()=>toast('Disalin!','success'))">[LIST] Copy</button>
          <button class="btn btn-success btn-sm" onclick="shareAppWA()">[CHAT] Share WA</button>
        </div>
      </div>
      <div class="card">
        <div class="card-title mb-16">[INSTALL] Install / Download Aplikasi</div>
    ...
    <div class="card">
      <div class="card-title mb-16">[LINK] Share Link Download ke Karyawan</div>
      ...
      <div class="flex gap-8 flex-wrap">
        <button class="btn btn-primary btn-sm" onclick="navigator.clipboard?.writeText('${url}').then(()=>toast('Link disalin!','success'))">[LIST] Copy Link</button>
        <button class="btn btn-success btn-sm" onclick="shareAppWA()">[CHAT] Share via WhatsApp</button>
        <button class="btn btn-info btn-sm" onclick="shareAppBroadcast()">[BROADCAST] Broadcast ke Semua Karyawan</button>
  ...
  return `
    <div style="margin-top:16px;border-top:1px solid var(--border);padding-top:16px">
      <div style="text-align:center;margin-bottom:20px">
        <button class="btn btn-primary" id="btnInstallPWA" style="display:${showBtn};padding:14px 32px;font-size:1rem;border-radius:12px" onclick="triggerInstallPWA()">[INSTALL] Install Aplikasi Sekarang</button>
        <button class="btn btn-outline" style="padding:14px 32px;font-size:.9rem;border-radius:12px;margin-left:8px" onclick="triggerInstallPWA()">[INSTALL] Cara Install</button>
      </div>
      <div class="fw-700 mb-12" style="font-size:.9rem">[INSTALL] Install di Perangkat Anda:</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px">
        <div style="padding:12px;border:1px solid var(--border);border-radius:8px;text-align:center;cursor:pointer" onclick="triggerInstallPWA()">
          <div style="font-size:1.5rem">[ANDROID]</div>
          <div class="fw-700 text-sm mt-8">Android</div>
          <div class="text-xs" style="color:#666;margin-top:4px">Chrome → Menu (⋮) → "Install app"</div>
        </div>
        <div style="padding:12px;border:1px solid var(--border);border-radius:8px;text-align:center;cursor:pointer" onclick="triggerInstallPWA()">
          <div style="font-size:1.5rem">[IOS]</div>
          <div class="fw-700 text-sm mt-8">iOS</div>
          <div class="text-xs" style="color:#666;margin-top:4px">Safari → Share ([UP]) → "Add to Home Screen"</div>
        </div>
        <div style="padding:12px;border:1px solid var(--border);border-radius:8px;text-align:center;cursor:pointer" onclick="triggerInstallPWA()">
          <div style="font-size:1.5rem">[WIN]</div>
        ...
        <div style="padding:12px;border:1px solid var(--border);border-radius:8px;text-align:center;cursor:pointer" onclick="triggerInstallPWA()">
          <div style="font-size:1.5rem">[MAC]</div>
      ...
      <div style="margin-top:16px;padding:12px;background:#e8f5e9;border-radius:8px;border-left:4px solid var(--success)">
        <div class="text-sm fw-700 mb-4">[OK] Setelah install:</div>
...
// ── PORTAL KARYAWAN ───────────────────────────────────────────
async function renderPortal(){const main=document.getElementById('mainContent');const u=currentUser;main.innerHTML=`<div class="page-title"><span>[HOME] Portal Saya</span></div><div class="card" style="border-left:4px solid var(--primary)"><div class="flex gap-16" style="align-items:center"><div style="width:60px;height:60px;font-size:1.5rem;background:var(--primary);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center">${u.nama.charAt(0)}</div><div><div class="fw-700" style="font-size:1.1rem">${escHtml(u.nama)}</div><div class="text-sm" style="color:#999">${escHtml(u.departemen||'-')} • ${escHtml(u.role)}</div><div class="text-xs" style="color:#999">NIP: ${escHtml(u.nip||'-')}</div></div></div></div><div class="stats-grid"><div class="stat-card" style="cursor:pointer" onclick="navigateTo('portal-absensi')"><div class="stat-icon">[PIN]</div><div class="stat-value" id="pAbsen">-</div><div class="stat-label">Kehadiran Bulan Ini</div></div><div class="stat-card" style="cursor:pointer" onclick="navigateTo('portal-cuti')"><div class="stat-icon">[LEAVE]</div><div class="stat-value" id="pCuti">-</div><div class="stat-label">Sisa Cuti</div></div><div class="stat-card" style="cursor:pointer" onclick="navigateTo('inbox')"><div class="stat-icon">[INBOX]</div><div class="stat-value" id="pInbox">-</div><div class="stat-label">Inbox Meeting</div></div></div><div class="card"><div class="card-title">[ANNOUNCE] Pengumuman</div><div id="portalAnnounce">Loading...</div></div><div class="card"><div class="card-title">[INSTALL] Download / Install Aplikasi</div><p class="text-sm mb-8" style="color:#666">Install aplikasi ini di perangkat Anda untuk akses lebih cepat.</p>${renderDownloadAppSection()}</div>`;
...
async function loadJobdesk(){
  ...
  if(!snap.empty){
    ...
    if(p.deskripsi)html+=`<div class="mb-16"><div class="fw-700 mb-8">[NOTE] Deskripsi Pekerjaan</div><div class="text-sm" style="white-space:pre-wrap;line-height:1.8">${escHtml(p.deskripsi)}</div></div>`;
    if(p.tanggungJawab)html+=`<div class="mb-16"><div class="fw-700 mb-8">[OK] Tanggung Jawab</div><ul style="padding-left:20px;font-size:.85rem;line-height:2">${p.tanggungJawab.split('\n').filter(x=>x.trim()).map(x=>'<li>'+escHtml(x.trim())+'</li>').join('')}</ul></div>`;
    ...
    if(p.kpi)html+=`<div class="mb-16"><div class="fw-700 mb-8">[UP] Target KPI</div><div class="text-sm" style="white-space:pre-wrap">${escHtml(p.kpi)}</div></div>`;
    html+=`</div>`;
  } else {
...
function renderPortalPeraturan(){document.getElementById('mainContent').innerHTML=`<div class="page-title"><span>[SCROLL] Peraturan Perusahaan</span></div><div class="card">${renderPeraturanHTML(true)}</div>`;}
...
async function modalJobdeskPilih(){
  ...
  openModal(`<div class="modal-title">[LIST] Pilih Karyawan untuk Atur Jobdesk</div>
...
async function modalJobdesk(karyawanId, nama, posisi) {
  ...
  openModal(`<div class="modal-title">[LIST] Jobdesk: ${escHtml(nama)}</div>
    ...
    <div class="flex gap-8"><button class="btn btn-primary" onclick="simpanJobdesk('${karyawanId}','${docId||''}')">[SAVE] Simpan Jobdesk</button>${docId?`<button class="btn btn-danger" onclick="hapusDoc('hrd_jobdesk','${docId}','jobdesk-mgmt')">[DEL] Hapus</button>`:''}</div>`,true);
}
...
function editKasbonDoc(id){db.collection('hrd_kasbon').doc(id).get().then(d=>{const p=d.data();openModal(`<div class="modal-title">[EDIT] Edit Kasbon/Loan</div><div class="grid-2"><div class="form-group"><label>Nama</label><input class="form-control" id="ekNama" value="${escHtml(p.nama||'')}"></div><div class="form-group"><label>Jenis</label><select class="form-control" id="ekJenis"><option ${p.jenis==='Kasbon'?'selected':''}>Kasbon</option><option ${p.jenis==='Pinjaman Karyawan'?'selected':''}>Pinjaman Karyawan</option></select></div></div><div class="grid-2"><div class="form-group"><label>Jumlah</label><input class="form-control" type="number" id="ekJumlah" value="${p.jumlah||0}"></div><div class="form-group"><label>Cicilan (bln)</label><input class="form-control" type="number" id="ekCicilan" value="${p.cicilan||1}"></div></div><div class="form-group"><label>Status</label><select class="form-control" id="ekStatus"><option value="pending" ${p.status==='pending'?'selected':''}>Pending</option><option value="approved" ${p.status==='approved'?'selected':''}>Approved</option><option value="rejected" ${p.status==='rejected'?'selected':''}>Rejected</option><option value="lunas" ${p.status==='lunas'?'selected':''}>Lunas</option></select></div><button class="btn btn-primary" onclick="updateKasbonDoc('${id}')">[SAVE] Simpan</button><button class="btn btn-danger" style="margin-left:8px" onclick="hapusDoc('hrd_kasbon','${id}','kasbon')">[DEL] Hapus</button>`);});}
...
async function renderPortalShare() {
  ...
  main.innerHTML = `<div class="page-title"><span>[INSTALL] Download Aplikasi</span></div>
    <div class="card mb-16"><div class="card-title">[IMPORT] Link Aplikasi</div><p class="text-sm mb-16" style="color:#666">Bagikan link ini ke semua karyawan agar mereka bisa mengakses portal lewat browser atau menginstal sebagai aplikasi PWA di Android, iOS, Windows, dan Mac.</p>
      <div class="form-group"><label>Link Download</label><input class="form-control" readonly value="${downloadLink}" id="portalAppLink"></div>
      <div class="flex gap-8"><button class="btn btn-primary btn-sm" onclick="copyDownloadLink()">[LIST] Salin Link</button><button class="btn btn-success btn-sm" onclick="shareDownloadWhatsApp()">[CHAT] Share WA</button><button class="btn btn-info btn-sm" onclick="shareDownloadEmail()">[MAIL] Email</button></div>
    </div>
    <div class="grid-2"><div class="card"><div class="card-title mb-16">[PHONE] Android</div><p class="text-sm" style="color:#666">Buka link di browser Chrome, lalu pilih menu dan "Add to Home screen". Aplikasi akan terpasang seperti biasa.</p></div><div class="card"><div class="card-title mb-16">[IOS] iOS</div><p class="text-sm" style="color:#666">Buka link di Safari, tap tombol Share lalu pilih "Add to Home Screen". Jika diminta, pilih "Add".</p></div></div>
    <div class="grid-2"><div class="card"><div class="card-title mb-16">[PC] Windows</div><p class="text-sm" style="color:#666">Buka link di browser Edge/Chrome, lalu pilih install PWA dari menu browser atau toolbar.</p></div><div class="card"><div class="card-title mb-16"> Mac</div><p class="text-sm" style="color:#666">Buka link di browser Safari/Chrome, lalu gunakan opsi "Add to Home Screen" / "Install App".</p></div></div>`;
}
...
async function loadLamaranMasuk() {
  ...
  if (snap.empty) html = '<p class="text-sm" style="color:#999">Belum ada lamaran masuk</p>';
    ...
    snap.forEach(d => {
      ...
      html += `<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.posisi||'-')}</td><td>${escHtml(p.email||'-')}</td><td>${formatDate(p.createdAt)}</td><td><span class="badge ${badge}">${p.status||'baru'}</span></td><td><button class="btn btn-xs btn-info" onclick="detailLamaran('${d.id}')">[VIEW]</button> <button class="btn btn-xs btn-success" onclick="updateLamaran('${d.id}','diterima')">[OK]</button> <button class="btn btn-xs btn-danger" onclick="updateLamaran('${d.id}','ditolak')">[X]</button></td></tr>`;
    });
    html += '</tbody></table></div>';
  }
...
function detailLamaran(id) {
  db.collection('hrd_lamaran').doc(id).get().then(d => {
    const p = d.data();
    openModal(`<div class="modal-title">[DOC] Detail Lamaran</div>
      <div class="grid-2 mb-16">
...
async function renderPublicPortalCalon() {
  document.getElementById('app').innerHTML = `
    <div style="background:linear-gradient(135deg,var(--primary-dark),var(--primary));color:#fff;padding:24px 20px;text-align:center">
      <div style="font-size:1.4rem;font-weight:700">[OFFICE] LPK IJEF CORP</div>
      <div style="font-size:.9rem;opacity:.8;margin-top:4px">Portal Calon Karyawan</div>
    </div>
    <div style="max-width:900px;margin:0 auto;padding:20px">
      <div class="tabs" id="calonTabs">
        <div class="tab active" onclick="loadCalonTab('lowongan')">[NOTE] Lowongan</div>
        <div class="tab" onclick="loadCalonTab('lamar')">[SEND] Kirim Lamaran</div>
        <div class="tab" onclick="loadCalonTab('tentang')">[OFFICE] Tentang Perusahaan</div>
        <div class="tab" onclick="loadCalonTab('peraturan')">[SCROLL] Peraturan</div>
      </div>
...
async function loadCalonTab(tab) {
  ...
  if (tab === 'lowongan') {
    ...
    let html = '<div class="card"><div class="card-title mb-16">[NOTE] Lowongan Tersedia</div>';
    if (snap.empty) html += '<div class="empty-state"><div class="icon">[NOTE]</div><p>Saat ini belum ada lowongan terbuka</p></div>';
    else {
      snap.forEach(d => {
        const p = d.data();
        html += `<div style="padding:16px;border:1px solid var(--border);border-radius:8px;margin-bottom:12px;border-left:4px solid var(--primary)">
          ...
          <button class="btn btn-primary btn-sm mt-8" onclick="loadCalonTab('lamar')">[SEND] Lamar Posisi Ini</button>
        </div>`;
      });
    }
    html += '</div>';
    ...
    el.innerHTML = `<div class="card">
      <div class="card-title mb-16">[SEND] Form Lamaran Kerja</div>
      ...
      <button class="btn btn-primary" style="width:100%;padding:12px" onclick="kirimLamaran()">[SEND] Kirim Lamaran</button>
    </div>`;

  } else if (tab === 'tentang') {
    el.innerHTML = `<div class="card">
      <div class="card-title mb-16">[OFFICE] Tentang LPK IJEF CORP</div>
      <div style="text-align:center;margin-bottom:20px">
        <div style="font-size:2rem">[OFFICE]</div>
      ...
      <div class="text-sm" style="line-height:1.8">
        ...
        <div class="grid-2 mt-8">
          ...
          <div class="stat-card"><div class="stat-icon">[START]</div><div class="fw-700">Inovasi</div><div class="text-xs">Terus belajar dan berkembang</div></div>
    ...
    el.innerHTML = `<div class="card"><div class="card-title mb-16">[SCROLL] Peraturan Perusahaan — LPK IJEF CORP</div><p class="text-sm mb-16" style="color:#666">Berikut ringkasan peraturan perusahaan yang berlaku bagi seluruh karyawan.</p>${renderPeraturanHTML(true)}</div>`;
  }
}
...
async function kirimLamaran() {
  ...
  document.getElementById('calonContent').innerHTML = `<div class="card text-center" style="padding:40px">
    ...
    <button class="btn btn-primary mt-16" onclick="loadCalonTab('lowongan')">[NOTE] Lihat Lowongan Lain</button>
  </div>`;
