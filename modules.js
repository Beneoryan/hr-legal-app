...
async function renderKandidat(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>🧑‍💼 Kandidat</span><div class="flex gap-8"><button class="btn btn-primary btn-sm" onclick="modalKandidat()">+ Tambah</button><button class="btn btn-success btn-sm" onclick="syncDiscCalonToKandidat()">🔄 Sinkron dari DISC</button></div></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Nama</th><th>Posisi</th><th>Stage</th><th>DISC</th><th>Aksi</th></tr></thead><tbody id="tblKand"></tbody></table></div></div>`;const snap=await db.collection('hrd_kandidat').orderBy('createdAt','desc').get();let h='';if(snap.empty)h='<tr><td colspan="5" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();h+=`<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.posisi||'-')}</td><td><span class="badge badge-info">${p.stage||'Applied'}</span></td><td>${p.discPattern?`<span class="badge badge-primary">${escHtml(p.discPattern)}</span>`:'-'}</td><td><button class="btn btn-xs btn-info" onclick="modalKandidat('${d.id}')">✏️</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_kandidat','${d.id}','kandidat')">🗑️</button></td></tr>`;});document.getElementById('tblKand').innerHTML=h;}
...
async function viewDiscResult(id){
  ...
  openModal(`<div class="modal-title">📊 D.I.S.C. Personality System Graph Page</div>
    ...
    ${career?`<div style="background:#e8f5e9;border-radius:6px;padding:8px;margin-bottom:10px"><div style="font-size:.7rem;font-weight:700;color:#1b5e20;margin-bottom:3px">💼 Rekomendasi Bidang Pekerjaan</div><div style="font-size:.73rem;color:#2e7d32;line-height:1.4">${escHtml(career)}</div></div>`:''}
