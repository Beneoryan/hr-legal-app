...
function renderPeraturanHTML(compact){let h='<div style="margin-bottom:24px;padding:16px 20px;background:#f3f4f6;border-radius:10px;border:1px solid #e0e0e0"><div style="font-weight:700;font-size:1rem;color:#1a237e;margin-bottom:12px;text-align:center">DAFTAR ISI</div><ol style="padding-left:20px;font-size:0.85rem;line-height:2.2;list-style:none;padding:0">';PERATURAN_PERUSAHAAN.bab.forEach(bab=>{h+=`<li style="margin-bottom:4px"><a href="#bab-${bab.nomor}" onclick="document.getElementById('bab-detail-${bab.nomor}').open=true" style="color:#1a237e;text-decoration:none;font-weight:600">BAB ${bab.nomor} - ${bab.judul}</a><span style="color:#888;font-size:0.8rem;margin-left:8px">(`;const pn=bab.pasal.map(p=>p.nomor);h+=pn.length===1?`Pasal ${pn[0]}`:`Pasal ${pn[0]}-${pn[pn.length-1]}`;h+=')</span></li>';});h+='</ol></div>';PERATURAN_PERUSAHAAN.bab.forEach(bab=>{h+=`<details id="bab-detail-${bab.nomor}" style="margin-bottom:${compact?'12px':'18px'};border:1px solid #e0e0e0;border-radius:10px;overflow:hidden"><summary style="background:#1a237e;color:white;padding:${compact?'10px 14px':'12px 18px'};font-weight:700;font-size:${compact?'0.88rem':'0.95rem'};cursor:pointer;user-select:none"><span id="bab-${bab.nomor}"></span>BAB ${bab.nomor}: ${bab.judul}</summary><div style="padding:${compact?'12px':'16px 20px'}">`;bab.pasal.forEach(pasal=>{h+=`<div style="margin-bottom:14px;padding:12px 16px;background:#f8f9ff;border-radius:8px;border-left:3px solid #1a237e"><div style="font-weight:700;color:#1a237e;margin-bottom:8px;font-size:.9rem">Pasal ${pasal.nomor}: ${pasal.judul}</div>`;if(pasal.isi){h+=`<ol style="padding-left:20px;font-size:0.83rem;line-height:1.9">`;pasal.isi.forEach(i=>{h+=`<li style="margin-bottom:4px">${escHtml(i)}</li>`;});h+=`</ol>`;}if(pasal.sub){pasal.sub.forEach(sub=>{h+=`<div style="margin-top:10px"><div style="font-weight:600;font-size:.84rem;margin-bottom:6px;color:#283593">${escHtml(sub.label)}</div><ul style="padding-left:20px;font-size:.82rem;line-height:1.9">`;sub.items.forEach(i=>{h+=`<li style="margin-bottom:2px">${escHtml(i)}</li>`;});h+=`</ul></div>`;});}if(pasal.catatan){h+=`<div style="margin-top:10px;padding:8px 12px;background:#fff8e1;border-radius:6px;font-size:.8rem;color:#e65100;border:1px solid #ffe0b2"><b>Catatan:</b> ${escHtml(pasal.catatan)}</div>`;}if(pasal.tabel){h+=`<div class="table-wrap" style="margin-top:12px;overflow-x:auto"><table style="width:100%;border-collapse:collapse"><thead><tr>`;pasal.tabel.headers.forEach(x=>{h+=`<th style="font-size:.78rem;padding:8px 10px;background:#e8eaf6;border:1px solid #c5cae9;text-align:left">${escHtml(x)}</th>`;});h+=`</tr></thead><tbody>`;pasal.tabel.rows.forEach(r=>{h+=`<tr>`;r.forEach(c=>{h+=`<td style="font-size:.8rem;padding:6px 10px;border:1px solid #e0e0e0">${escHtml(c)}</td>`;});h+=`</tr>`;});h+=`</tbody></table></div>`;}h+=`</div>`;});h+=`</div></details>`;});return h;}
...
async function simpanSPPD(noSPPD){
  ...
  const _hari=Math.ceil((new Date(data.tanggalSelesai)-new Date(data.tanggalMulai))/(1000*60*60*24)+1);
