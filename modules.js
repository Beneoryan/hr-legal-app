...
function modalGaji(){openModal(`<div class="modal-title">Generate Slip Gaji</div><div class="grid-2"><div class="form-group"><label>Karyawan</label><input class="form-control" id="gjNama"></div><div class="form-group"><label>Periode</label><input class="form-control" type="month" id="gjPeriode" value="${monthStr()}"></div></div>
    ...
    <button class="btn btn-primary mt-16" onclick="simpanGaji()">💾 Simpan Slip</button><button class="btn btn-outline mt-16" style="margin-left:8px" onclick="autoFillGajiFromKaryawan()">🔄 Auto-fill dari Data Karyawan</button>`,true);}
function hitungGaji(){const pokok=Number(document.getElementById('gjPokok').value)||0;const tJabatan=Number(document.getElementById('gjTunjJabatan').value)||0;const tTransport=Number(document.getElementById('gjTunjTransport').value)||0;const tMakan=Number(document.getElementById('gjTunjMakan').value)||0;const tKom=Number(document.getElementById('gjTunjKom').value)||0;const lembur=Number(document.getElementById('gjLembur').value)||0;const bonus=Number(document.getElementById('gjBonus').value)||0;const tLain=Number(document.getElementById('gjTunjLain').value)||0;const bpjsKes=Number(document.getElementById('gjBPJSKes').value)||0;const bpjsTK=Number(document.getElementById('gjBPJSTK').value)||0;const potLain=Number(document.getElementById('gjPotongan').value)||0;const kasbon=Number(document.getElementById('gjKasbon').value)||0;
  const totalTunjangan=tJabatan+tTransport+tMakan+tKom+lembur+bonus+tLain;const bruto=pokok+totalTunjangan;const totalPotongan=bpjsKes+bpjsTK+potLain+kasbon;
  // PPH21 progressive: 5% (0-60jt), 15% (60-250jt), 25% (250-500jt), 30% (500jt-5M), 35% (>5M)
  const tahunan=(bruto-bpjsKes-bpjsTK)*12;let pphTahunan=0;
  ...
  const pph21=Math.round(pphTahunan/12);const total=bruto-totalPotongan-pph21;
  ...
  window._gajiCalc={pph21,total,bruto,totalTunjangan,totalPotongan};}
async function autoFillGajiFromKaryawan(){const nama=document.getElementById('gjNama').value.trim();if(!nama)return toast('Isi nama karyawan dulu','warning');const snap=await db.collection('hrd_karyawan').where('nama','==',nama).limit(1).get();if(snap.empty)return toast('Karyawan tidak ditemukan','warning');const k=snap.docs[0].data();document.getElementById('gjPokok').value=k.gajiPokok||0;const gaji=k.gajiPokok||0;document.getElementById('gjBPJSKes').value=Math.round(gaji*0.01);document.getElementById('gjBPJSTK').value=Math.round(gaji*0.02);hitungGaji();toast('Data gaji dari karyawan dimuat','success');}
async function simpanGaji(){const tJab=Number(document.getElementById('gjTunjJabatan').value)||0;const tTrans=Number(document.getElementById('gjTunjTransport').value)||0;const tMakan=Number(document.getElementById('gjTunjMakan').value)||0;const tKom=Number(document.getElementById('gjTunjKom').value)||0;const lembur=Number(document.getElementById('gjLembur').value)||0;const bonus=Number(document.getElementById('gjBonus').value)||0;const tLain=Number(document.getElementById('gjTunjLain').value)||0;const data={nama:document.getElementById('gjNama').value,periode:document.getElementById('gjPeriode').value,gajiPokok:Number(document.getElementById('gjPokok').value)||0,tunjangan:tJab+tTrans+tMakan+tKom+lembur+bonus+tLain,tunjJabatan:tJab,tunjTransport:tTrans,tunjMakan:tMakan,tunjKomunikasi:tKom,lembur,bonus,tunjLain:tLain,bpjsKesehatan:Number(document.getElementById('gjBPJSKes').value)||0,bpjsTK:Number(document.getElementById('gjBPJSTK').value)||0,potongan:Number(document.getElementById('gjPotongan').value)||0,kasbon:Number(document.getElementById('gjKasbon').value)||0,pph21:window._gajiCalc?.pph21||0,totalBersih:window._gajiCalc?.total||0,createdAt:new Date().toISOString()};if(!data.nama)return toast('Nama wajib','warning');await db.collection('hrd_penggajian').add(data);closeModalDirect();toast('Slip disimpan','success');renderPenggajian();}
...
async function viewDiscResult(id){
  ...
  // Positive/Negative traits - fallback to pattern-based lookup if not stored
  const discProfiles={'D':{pos:['Individualis','High Motivation','Efektif','Percaya Diri','Kreatif','Leader'],neg:['Ego Tinggi','Kurang Sensitif','Agresif','Terlalu Dominan'],career:'Attorney, Sales Representative, Production Director, Strategic Planning, Self-Employment.'},'I':{pos:['Antusias','Optimis','Persuasif','Ramah','Inspirasional'],neg:['Emosional','Impulsif','Kurang fokus','Tidak terorganisir'],career:'Public Relations, Lecturing, Advertising, Hospitality, Human Resources, Travel Agent.'},'S':{pos:['Stabil','Sabar','Loyal','Pendengar baik','Dapat diandalkan'],neg:['Anti Perubahan','Sulit Adaptasi','Menghindari Konflik','Lambat Memutuskan'],career:'Administrative Work, Accounting, Research, Retail, Service.'},'C':{pos:['Detail','Analitis','Rapi','Organized','Sistematis'],neg:['Pendiam','Anti Kritik','Kaku','Terlalu Detail'],career:'Planner, Engineer, IT Management, Quality Controller, Accountant.'},'D-I':{pos:['Pekerja Keras','Leader','Tegas','Logis','Imajinatif'],neg:['Cepat Bosan','Dingin','Kurang Pergaulan','Anti Aturan'],career:'General Management, Sales Management, Marketing, Consultancy.'},'D-S':{pos:['Objektif','Analitis','Mandiri','Komitmen target','Stabil','Ulet'],neg:['Menghindari Konflik','Kurang fleksibel','Terlalu mandiri'],career:'Project Management, Researcher, Systems Analyst, Programmer, IT.'},'D-C':{pos:['Tekun','Sensitif','Keputusan kuat','Kreatif','Reaksi cepat'],neg:['Perfeksionis','Lambat keputusan','Terlalu fokus ketepatan'],career:'Engineering, Actuaries, Planning, Accountancy, Quality Control.'},'I-S':{pos:['Hangat','Simpati','Tenang','Pendengar baik','Toleran','Penjaga damai'],neg:['Kurang tegas','Terlalu toleran','Menerima kritik sebagai serangan'],career:'Personnel, Welfare, Training, Psychologist, Nursing, Social Work.'},'I-C':{pos:['Ramah','Suka berteman','Dapat mengendalikan diri','Perfeksionis alamiah'],neg:['Kadang salah menilai','Terlalu optimis','Kurang fokus tugas'],career:'Teaching, Training, Specialist Selling, Project Engineer, Marketing.'},'S-I':{pos:['Hangat','Simpati','Pendengar baik','Toleran','Penjaga damai','Moderat'],neg:['Kurang tegas','Terlalu toleran','Sulit membuat keputusan'],career:'Personnel, Training, Hotelier, Travel Agent, Psychologist, Nurse.'},'S-C':{pos:['Detail','Empati','Loyal','Teliti','Peduli'],neg:['Anti Kritik','Sulit Adaptasi','Introvert','Pendendam'],career:'Office Manager, Planner, Accountant, Programmer, Health Care.'},'C-D':{pos:['Sensitif','Berorientasi tugas','Kukuh','Efektif','Berdasar fakta'],neg:['Dingin','Tidak berperasaan','Menjaga jarak','Tidak mudah percaya'],career:'Engineering, Research, Planning, Accountancy, Quality Control.'},'C-S':{pos:['Detail','Sistematik','Bijaksana','Diplomatis','Akurasi tinggi'],neg:['Menghindari Konflik','Lambat Memutuskan','Anti Perubahan'],career:'Researcher, Engineer, Statistician, IT Management, Planner.'}};
  function getProfileData(pat){if(discProfiles[pat])return discProfiles[pat];const p=pat?pat.split('-'):[];if(p.length>=2&&discProfiles[p[0]+'-'+p[1]])return discProfiles[p[0]+'-'+p[1]];if(discProfiles[p[0]])return discProfiles[p[0]];return{pos:[],neg:[],career:''};}
  const pData=getProfileData(r.pattern||'');
  const posTraits=r.positiveTraits&&r.positiveTraits.length?r.positiveTraits:pData.pos;
  const negTraits=r.negativeTraits&&r.negativeTraits.length?r.negativeTraits:pData.neg;
  const career=r.career||pData.career;
  let posH='',negH='';
  posTraits.forEach(t=>{posH+=`<div style="padding:3px 0;font-size:.78rem;color:#2e7d32">✅ ${escHtml(t)}</div>`;});
  negTraits.forEach(t=>{negH+=`<div style="padding:3px 0;font-size:.78rem;color:#c62828">⚠️ ${escHtml(t)}</div>`;});
  // KPI Score
  ...
  openModal(`<div class="modal-title">📊 Detail Lengkap Hasil DISC</div>
    ...
    <div style="background:#f8f9ff;border-radius:8px;padding:12px;margin-bottom:12px;font-size:.82rem;line-height:1.6"><strong>📝 Deskripsi:</strong><br>${escHtml(r.description||getDiscDesc(r.pattern)||'Profil kepribadian yang menggabungkan beberapa karakteristik dominan. Silakan lihat sifat positif dan negatif untuk detail lebih lanjut.')}</div>
    ...
    ${career?`<div style="background:#e8f5e9;border-radius:8px;padding:10px;margin-bottom:12px"><div style="font-size:.78rem;font-weight:700;color:#1b5e20;margin-bottom:4px">💼 Rekomendasi Karir</div><div style="font-size:.78rem;color:#2e7d32;line-height:1.5">${escHtml(career)}</div></div>`:''}
