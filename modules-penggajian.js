...
function filterGajiTable() {
  ...
  if (!filtered.length) h = '<tr><td colspan="12" class="text-center">Tidak ada data</td></tr>';
    ...
    filtered.forEach((p) => {
      ...
      totPPH += p.pph21 || 0;
      count++;
      h += `<tr><td><input type="checkbox" class="gaji-cb" value="${p.id}"></td><td class="fw-700">${escHtml(p.nama)}</td><td>${formatCurrency(p.gajiPokok)}</td><td>${formatCurrency(p.tunjangan)}</td><td>${formatCurrency(p.insentif || 0)}</td><td>${formatCurrency(p.reimbursement || 0)}</td><td>${formatCurrency(p.lembur || 0)}</td><td>${formatCurrency(p.potongan)}</td><td>${formatCurrency(p.kasbon || 0)}</td><td>${formatCurrency(p.pph21)}</td><td class="fw-700">${formatCurrency(p.totalBersih)}</td><td><button class="btn btn-xs btn-info" onclick="lihatSlip('${p.id}')">📄</button> <button class="btn btn-xs btn-warning" onclick="editGaji('${p.id}')">✏️</button> <button class="btn btn-xs btn-danger" onclick="hapusDoc('hrd_penggajian','${p.id}','penggajian')">🗑️</button></td></tr>`;
    });
