...
async function simpanReimburse(){const data={nama:document.getElementById('rbNama').value,kategori:document.getElementById('rbKat').value,jumlah:Number(document.getElementById('rbJumlah').value)||0,keterangan:document.getElementById('rbKet').value,status:'pending',userId:currentUser.id,createdAt:new Date().toISOString()};if(!data.jumlah)return toast('Jumlah wajib','warning');await db.collection('hrd_reimbursement').add(data);await sendNotification('admin','📋 Reimbursement Baru',`${currentUser.nama} mengajukan reimbursement ${data.kategori}`,'approval-center');closeModalDirect();toast('Diajukan','success');renderReimbursement();}
async function approveReimb(id,status){await db.collection('hrd_reimbursement').doc(id).update({status,approvedBy:currentUser.nama});const doc=await db.collection('hrd_reimbursement').doc(id).get();const p=doc.data();await sendNotification(p.userId,status==='approved'?'✅ Reimbursement Disetujui':'❌ Reimbursement Ditolak',`Pengajuan reimbursement ${p.kategori} Anda telah ${status==='approved'?'disetujui':'ditolak'}`,'reimbursement');toast('Updated','success');renderReimbursement();}
...
async function simpanKasbon(){const jumlah=Number(document.getElementById('kbJumlah').value)||0;const cicilan=Number(document.getElementById('kbCicilan').value)||1;if(!jumlah)return toast('Jumlah wajib','warning');const data={nama:document.getElementById('kbNama').value,jenis:document.getElementById('kbJenis').value,jumlah,cicilan,angsuran:Math.ceil(jumlah/cicilan),sudahBayar:0,keterangan:document.getElementById('kbKet').value,status:'pending',userId:currentUser.id,createdAt:new Date().toISOString()};await db.collection('hrd_kasbon').add(data);await sendNotification('admin','📋 Kasbon Baru',`${currentUser.nama} mengajukan ${data.jenis} Rp ${jumlah.toLocaleString('id-ID')}`,'approval-center');closeModalDirect();toast('Diajukan','success');renderKasbon();}
async function approveKasbon(id,status){await db.collection('hrd_kasbon').doc(id).update({status,approvedBy:currentUser.nama,approvedAt:new Date().toISOString()});const doc=await db.collection('hrd_kasbon').doc(id).get();const p=doc.data();await sendNotification(p.userId,status==='aktif'?'✅ Kasbon Disetujui':'❌ Kasbon Ditolak',`Pengajuan ${p.jenis} Anda telah ${status==='aktif'?'disetujui':'ditolak'}`,'kasbon');toast('Updated','success');renderKasbon();}
