...
async function loadRekapGrid(){
  ...
  const cutiMap={};
  cutiSnap.forEach(d=>{const c=d.data();if(c.status!=='approved')return;if(!c.mulai||!c.selesai)return;const uid=c.userId||'';const nama=(c.nama||'').toLowerCase();const start=new Date(c.mulai);const end=new Date(c.selesai);for(let dt=new Date(start);dt<=end;dt.setDate(dt.getDate()+1)){const ds=dt.toISOString().split('T')[0];if(ds>=startDate&&ds<=endDate){const day=dt.getDate();if(uid){if(!cutiMap[uid])cutiMap[uid]={};cutiMap[uid][day]=c.jenis||'Cuti';}if(nama){if(!cutiMap[nama])cutiMap[nama]={};cutiMap[nama][day]=c.jenis||'Cuti';}}}});
  ...
  const dinasLuarMap={};
  dinasLuarSnap.forEach(d=>{const dl=d.data();if(dl.status!=='approved')return;const uid=dl.userId||'';const dlNama=(dl.nama||'').toLowerCase();const startD=dl.tanggalMulai||dl.tanggal;const endD=dl.tanggalSelesai||dl.tanggal;if(!startD)return;const endTime=new Date(endD||startD).getTime();let maxIter=366;for(let dt=new Date(startD);dt.getTime()<=endTime;dt.setDate(dt.getDate()+1)){if(--maxIter<0)break;const ds=dt.toISOString().split('T')[0];if(ds>=startDate&&ds<=endDate){const day=dt.getDate();if(uid){if(!dinasLuarMap[uid])dinasLuarMap[uid]={};dinasLuarMap[uid][day]=true;}if(dlNama){if(!dinasLuarMap[dlNama])dinasLuarMap[dlNama]={};dinasLuarMap[dlNama][day]=true;}}}});
  sppdSnap.forEach(d=>{const sp=d.data();if(sp.status!=='approved')return;const uid=sp.userId||'';const spNama=(sp.nama||'').toLowerCase();if(!sp.tanggalMulai)return;const endTime=new Date(sp.tanggalSelesai||sp.tanggalMulai).getTime();let maxIter=366;for(let dt=new Date(sp.tanggalMulai);dt.getTime()<=endTime;dt.setDate(dt.getDate()+1)){if(--maxIter<0)break;const ds=dt.toISOString().split('T')[0];if(ds>=startDate&&ds<=endDate){const day=dt.getDate();if(uid){if(!dinasLuarMap[uid])dinasLuarMap[uid]={};dinasLuarMap[uid][day]=true;}if(spNama){if(!dinasLuarMap[spNama])dinasLuarMap[spNama]={};dinasLuarMap[spNama][day]=true;}}}});
