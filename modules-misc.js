'use strict';
// ── KPI ───────────────────────────────────────────────────────
async function renderKPI(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>📈 KPI & Penilaian</span><button class="btn btn-primary btn-sm" onclick="modalKPI()">+ Tambah</button></div><div style="margin-bottom:12px"><button type="button" class="btn btn-sm btn-info" onclick="document.getElementById('kpiInfoPanelAdmin').style.display=document.getElementById('kpiInfoPanelAdmin').style.display==='none'?'block':'none'">ℹ️ Info Formula KPI</button><div id="kpiInfoPanelAdmin" style="display:none;margin-top:12px;padding:12px;background:#f0f4ff;border-radius:8px;font-size:.82rem;line-height:1.6"><strong>Formula Penilaian KPI:</strong><br>• Produktivitas (25%), Kualitas (25%), Kedisiplinan (25%), Kerjasama (25%)<br>• Skor Murni = Rata-rata 4 komponen<br>• Setiap 1 penalty point mengurangi skor akhir sebesar 2 poin<br>• <strong>Skor Akhir = Skor Murni - (Total Penalty x 2)</strong><br><br><strong>Grade:</strong> A (≥90) | B (≥80) | C (≥70) | D (≥60) | E (&lt;60)</div></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Karyawan</th><th>Periode</th><th>Skor Murni</th><th>Penalty</th><th>Skor Akhir</th><th>Grade</th><th>Penilai</th></tr></thead><tbody id="tblKPI"></tbody></table></div></div>`;const [snap,penSnap]=await Promise.all([db.collection('hrd_kpi').get(),db.collection('hrd_penalty').get()]);const penaltyMap={};penSnap.forEach(d=>{const pe=d.data();const nm=pe.nama||'';penaltyMap[nm]=(penaltyMap[nm]||0)+(parseInt(pe.poin)||0);});let h='';if(snap.empty)h='<tr><td colspan="7" class="text-center">Belum ada</td></tr>';else snap.forEach(d=>{const p=d.data();const skorMurni=p.skorMurni!=null?p.skorMurni:p.skor;const livePenaltyPoin=penaltyMap[p.nama]||0;const liveDed=livePenaltyPoin*2;const liveSkor=Math.max(0,skorMurni-liveDed);const grade=liveSkor>=90?'A':liveSkor>=80?'B':liveSkor>=70?'C':liveSkor>=60?'D':'E';h+=`<tr><td class="fw-700">${escHtml(p.nama)}</td><td>${escHtml(p.periode)}</td><td>${skorMurni}/100</td><td>${liveDed>0?'<span class="badge badge-danger">-'+liveDed+'</span>':'<span class="badge badge-success">0</span>'}</td><td><span class="badge badge-${liveSkor>=80?'success':liveSkor>=60?'warning':'danger'}">${liveSkor}/100</span></td><td class="fw-700">${grade}</td><td>${escHtml(p.penilai||'-')}</td></tr>`;});document.getElementById('tblKPI').innerHTML=h;}
