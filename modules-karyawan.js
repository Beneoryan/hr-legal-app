...
async function modalLowongan(id){if(id){const d=await db.collection('hrd_lowongan').doc(id).get();await showLowForm(id,d.data()||{});}else await showLowForm(null,{});}
async function showLowForm(id,p){const[posSnap,deptSnap]=await Promise.all([db.collection('hrd_posisi').get(),db.collection('hrd_departemen').get()]);let posOpts='<option value="">-- Pilih Posisi --</option>';posSnap.forEach(d=>{const r=d.data();posOpts+=`<option value="${escHtml(r.nama)}" ${(p.posisi||'')===r.nama?'selected':''}>${escHtml(r.nama)}</option>`;});let deptOpts='<option value="">-- Pilih Departemen --</option>';deptSnap.forEach(d=>{const r=d.data();deptOpts+=`<option value="${escHtml(r.nama)}" ${(p.departemen||'')===r.nama?'selected':''}>${escHtml(r.nama)}</option>`;});openModal(`<div class="modal-title">${id?'Edit':'Tambah'} Lowongan</div><div class="grid-2"><div class="form-group"><label>Posisi</label><select class="form-control" id="lwPos">${posOpts}</select></div><div class="form-group"><label>Dept</label><select class="form-control" id="lwDept">${deptOpts}</select></div></div><div class="grid-2"><div class="form-group"><label>Status</label><select class="form-control" id="lwStatus"><option value="open" ${p.status==='open'?'selected':''}>Open</option><option value="closed" ${p.status==='closed'?'selected':''}>Closed</option></select></div><div class="form-group"><label>Deadline</label><input class="form-control" type="date" id="lwDead" value="${p.deadline||''}"></div></div><div class="form-group"><label>Deskripsi</label><textarea class="form-control" id="lwDesc">${escHtml(p.deskripsi||'')}</textarea></div><button class="btn btn-primary" onclick="simpanLowongan('${id||''}')">Simpan</button>`);}
