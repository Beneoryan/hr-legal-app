...
function renderResult(r){
  ...
  const dd={D:'<strong>berorientasi pada hasil (result-oriented), suka tantangan, dan mandiri</strong>. Ia memiliki pendekatan yang tegas dan efektif dalam memecahkan masalah',I:'<strong>ramah, optimis, persuasif, dan suka bersosialisasi</strong>. Ia memiliki kemampuan komunikasi yang sangat baik dan mampu memotivasi orang lain',S:'<strong>sabar, loyal, konsisten, dan kooperatif</strong>. Ia bekerja dengan baik dalam tim dan menghindari konflik',C:'<strong>perfeksionis, detail, rapi, dan terorganisir</strong>. Ia adalah pemikir logis yang membuat keputusan berdasarkan fakta dan data, bukan emosi'};
  const ddLow={D:'<strong>pendiam, menghindari konfrontasi, dan kurang agresif</strong>. Fokus utamanya bukan pada dominasi atau pengendalian situasi',I:'<strong>pendiam, dingin, menjaga jarak, dan kurang percaya pada orang lain</strong>. Fokus utamanya adalah pada tugas, bukan pada interaksi sosial atau membangun hubungan',S:'<strong>tidak sabar, suka perubahan, dan dinamis</strong>. Ia lebih suka tantangan baru daripada rutinitas',C:'<strong>spontan, fleksibel, dan tidak terlalu mengikuti aturan</strong>. Ia lebih mengutamakan kecepatan daripada kesempurnaan'};
  const nicknames={D:'Pemimpin Tegas',I:'Komunikator Handal',S:'Pendukung Setia',C:'Ahli Pemecah Masalah','D-I':'Pemimpin Visioner','D-S':'Strategis Mandiri','D-C':'Ahli Pemecah Masalah','I-S':'Diplomat Hangat','I-C':'Analis Kreatif','I-D':'Negosiator Ulung','S-I':'Penasihat Bijak','S-C':'Pelaksana Teliti','S-D':'Pekerja Tangguh','C-D':'Perancang Sistem','C-I':'Penilai Cermat','C-S':'Perfeksionis Terstruktur'};
  const nickname=nicknames[pattern]||nicknames[pattern.split('-').slice(0,2).join('-')]||nicknames[pattern.split('-')[0]]||'Profesional Unik';
  ...
  dominant.forEach(([t,v])=>{kesimp+=`<strong>Sisi ${t} (${dn[t]}) yang tinggi:</strong> Ini membuatnya menjadi pribadi yang ${dd[t]}.\n`;});
  weak.forEach(([t,v])=>{kesimp+=`<strong>Sisi ${t} (${dn[t]}) yang rendah:</strong> Ini menjelaskan mengapa ia digambarkan sebagai ${ddLow[t]}.\n`;});
