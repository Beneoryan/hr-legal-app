...
// ── PORTAL: KPI ───────────────────────────────────────────────
async function renderPortalKPI(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>📈 KPI Saya</span></div><div class="card mb-16" style="background:#f0f4ff;border:1px solid #d0d8f0"><div style="font-weight:700;margin-bottom:8px">ℹ️ Cara Perhitungan KPI</div><div style="font-size:.82rem;line-height:1.6">• 4 Komponen: Produktivitas (25%), Kualitas (25%), Kedisiplinan (25%), Kerjasama (25%)<br>• Skor Murni = Rata-rata 4 komponen<br>• Setiap 1 penalty point mengurangi skor akhir sebesar 2 poin<br>• <strong>Skor Akhir = Skor Murni - (Total Penalty x 2)</strong><br>• Grade: A (≥90) | B (≥80) | C (≥70) | D (≥60) | E (&lt;60)</div></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Periode</th><th>Skor</th><th>Penalty</th><th>Grade</th><th>Penilai</th><th>Catatan</th></tr></thead><tbody id="tblMyKPI"></tbody></table></div></div>`;const [snap,penSnap]=await Promise.all([db.collection('hrd_kpi').where('nama','==',currentUser.nama).get(),db.collection('hrd_penalty').where('nama','==',currentUser.nama).get()]);let livePenaltyPoin=0;penSnap.forEach(d=>{livePenaltyPoin+=(parseInt(d.data().poin)||0);});const liveDed=livePenaltyPoin*2;const items=[];snap.forEach(d=>{items.push(d.data());});items.sort((a,b)=>(b.createdAt||'').localeCompare(a.createdAt||''));let h='';if(!items.length)h='<tr><td colspan="6" class="text-center">Belum ada penilaian</td></tr>';else items.forEach(p=>{const skorMurni=p.skorMurni!=null?p.skorMurni:p.skor;const liveSkor=Math.max(0,skorMurni-liveDed);const grade=liveSkor>=90?'A':liveSkor>=80?'B':liveSkor>=70?'C':liveSkor>=60?'D':'E';const skorInfo=liveDed>0?`<span class="badge badge-${liveSkor>=80?'success':liveSkor>=60?'warning':'danger'}">${liveSkor}/100</span><br><span style="font-size:.72rem;color:#666">Murni: ${skorMurni}</span>`:`<span class="badge badge-${liveSkor>=80?'success':liveSkor>=60?'warning':'danger'}">${liveSkor}/100</span>`;h+=`<tr><td>${escHtml(p.periode||'-')}</td><td>${skorInfo}</td><td>${liveDed>0?'<span class="badge badge-danger">-'+liveDed+'</span>':'<span class="badge badge-success">0</span>'}</td><td class="fw-700">${grade}</td><td>${escHtml(p.penilai||'-')}</td><td style="font-size:.78rem">${escHtml(p.catatan||'-')}</td></tr>`;});document.getElementById('tblMyKPI').innerHTML=h;}
