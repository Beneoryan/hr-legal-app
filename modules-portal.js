...
// ── PORTAL: KPI ───────────────────────────────────────────────
async function renderPortalKPI(){const main=document.getElementById('mainContent');main.innerHTML=`<div class="page-title"><span>📈 KPI Saya</span></div><div class="card mb-16" style="background:#f0f4ff;border:1px solid #d0d8f0"><div style="font-weight:700;margin-bottom:8px">ℹ️ Cara Perhitungan KPI</div><div style="font-size:.82rem;line-height:1.6">• 4 Komponen: Produktivitas (25%), Kualitas (25%), Kedisiplinan (25%), Kerjasama (25%)<br>• Skor Murni = Rata-rata 4 komponen<br>• Setiap 1 penalty point mengurangi skor akhir sebesar 2 poin<br>• <strong>Skor Akhir = Skor Murni - (Total Penalty x 2)</strong><br>• Grade: A (≥90) | B (≥80) | C (≥70) | D (≥60) | E (&lt;60)</div></div><div id="penaltyInfoKPI"></div><div class="card"><div class="table-wrap"><table><thead><tr><th>Periode</th><th>Skor</th><th>Penalty</th><th>Grade</th><th>Penilai</th><th>Catatan</th></tr></thead><tbody id="tblMyKPI"></tbody></table></div></div>`;const [snap,penSnap]=await Promise.all([db.collection('hrd_kpi').get(),db.collection('hrd_penalty').get()]);let livePenaltyPoin=0;const myNamaLower=(currentUser.nama||'').toLowerCase().trim();const myId=currentUser.id||'';const myLinked=currentUser.linkedKaryawan||'';penSnap.forEach(d=>{const pe=d.data();if((pe.nama||'').toLowerCase().trim()===myNamaLower)livePenaltyPoin+=(parseInt(pe.poin)||0);});const liveDed=livePenaltyPoin*2;
// Show penalty summary card if user has penalties
if(livePenaltyPoin>0){const statusLabel=livePenaltyPoin>=10?'🔴 SP III':livePenaltyPoin>=7?'🟠 SP II':livePenaltyPoin>=4?'🟡 SP I':'⚪ Peringatan';document.getElementById('penaltyInfoKPI').innerHTML=`<div class="card mb-16" style="background:#fff3e0;border:1px solid #ffcc80;border-left:4px solid var(--warning)"><div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap"><div><span class="fw-700">⚠️ Penalty Point Anda:</span> <span class="badge badge-danger">${livePenaltyPoin} poin</span></div><div><span class="fw-700">Potongan KPI:</span> <span class="badge badge-danger">-${liveDed} poin</span></div><div><span class="fw-700">Status:</span> ${statusLabel}</div></div></div>`;}
const items=[];snap.forEach(d=>{const r=d.data();const rNama=(r.nama||'').toLowerCase().trim();const rId=r.karyawanId||r.userId||'';const namaMatch=rNama===myNamaLower||(myNamaLower&&rNama.includes(myNamaLower))||(myNamaLower&&myNamaLower.includes(rNama));const idMatch=(myId&&rId===myId)||(myLinked&&rId===myLinked);if(namaMatch||idMatch)items.push(r);});items.sort((a,b)=>(b.createdAt||'').localeCompare(a.createdAt||''));let h='';if(!items.length)h='<tr><td colspan="6" class="text-center">Belum ada penilaian</td></tr>';else items.forEach(p=>{const skorMurni=p.skorMurni!=null?p.skorMurni:p.skor;const liveSkor=Math.max(0,skorMurni-liveDed);const grade=liveSkor>=90?'A':liveSkor>=80?'B':liveSkor>=70?'C':liveSkor>=60?'D':'E';const skorInfo=liveDed>0?`<span class="badge badge-${liveSkor>=80?'success':liveSkor>=60?'warning':'danger'}">${liveSkor}/100</span><br><span style="font-size:.72rem;color:#666">Murni: ${skorMurni}</span>`:`<span class="badge badge-${liveSkor>=80?'success':liveSkor>=60?'warning':'danger'}">${liveSkor}/100</span>`;h+=`<tr><td>${escHtml(p.periode||'-')}</td><td>${skorInfo}</td><td>${liveDed>0?'<span class="badge badge-danger">-'+liveDed+'</span>':'<span class="badge badge-success">0</span>'}</td><td class="fw-700">${grade}</td><td>${escHtml(p.penilai||'-')}</td><td style="font-size:.78rem">${escHtml(p.catatan||'-')}</td></tr>`;});document.getElementById('tblMyKPI').innerHTML=h;}
